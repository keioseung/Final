...
with st.expander("🌐 프롬프트와 기반 내용 합치기"):
    ...
    if 'base_storage' in st.session_state and st.session_state.prompt_storage and st.session_state.base_storage:
        ...
        selected_prompt_idx = st.selectbox("프롬프트 선택", range(len(st.session_state.prompt_storage)), format_func=lambda x: prompt_options[x])
        selected_base_idx = st.selectbox("기반 내용 선택", range(len(st.session_state.base_storage)), format_func=lambda x: base_options[x])
        ...
        st.text_area("최종 질문", value=combined_question, height=200, disabled=False)
    else:
